...
function isRemotePuzzleSafe(row: Record<string, unknown>) {
  ...
  if (!Number.isInteger(rows) || !Number.isInteger(columns) || rows <= 0 || columns <= 0) return false;
...
export function SocialProvider({ children }: { children: ReactNode }) {
  const { mergeRemotePuzzles, puzzles, t } = useApp();
  ...
  const placed = puzzles.reduce((sum, puzzle) => sum + puzzle.session.pieces.filter((piece) => piece.isPlaced).length, 0);
  const xp = completed * 500 + placed;

  const loadRemoteProfile = useCallback(async (userId: string) => {
    if (!supabase) return;
    const { data } = await supabase.from("profiles").select("id, display_name, avatar_url, bio, xp").eq("id", userId).maybeSingle();
    if (!data) return;
    ...
    const { data: remotePuzzles } = await supabase.from("puzzles").select("id, name, difficulty, configuration, session, image_uri, completed_at, created_at, updated_at").eq("user_id", userId);
    if (remotePuzzles) mergeRemotePuzzles(remotePuzzles.flatMap((row) => {
      ...
      return [{
        ...
        session: { ...row.session, completedAt: row.completed_at ?? row.session.completedAt },
  ...
  const refreshFriends = useCallback(async () => {
    ...
    setFriends((data ?? []).map((friend: Record<string, unknown>) => ({
      id: String(friend.id),
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      void client.from("profiles").update({ xp, last_seen_at: new Date().toISOString() }).eq("id", userId);
      if (puzzles.length > 0) void (async () => {
        const rows = [];
        for (const puzzle of puzzles) {
          let imageUri = puzzle.imageUri.startsWith("http") ? puzzle.imageUri : uploadedPuzzleImages.get(puzzle.id) ?? null;
          if (!imageUri) {
            try {
              const image = await fetch(puzzle.imageUri).then((response) => response.arrayBuffer());
              const path = `${userId}/${puzzle.id}.jpg`;
              const { error: imageError } = await client.storage.from("puzzle-images").upload(path, image, { contentType: "image/jpeg", upsert: true });
              if (!imageError) {
                imageUri = client.storage.from("puzzle-images").getPublicUrl(path).data.publicUrl;
                uploadedPuzzleImages.set(puzzle.id, imageUri);
              }
            } catch { /* The JSON remains local until the image becomes readable again. */ }
          }
          rows.push({
            id: puzzle.id,
            user_id: userId,
      ...
      const progress = [
        ...
        { key: "no_hints", progress: puzzles.some((p) => p.session.completedAt && p.session.hintsUsed === 0) ? 100 : 0, unlocked: puzzles.some((p) => p.session.completedAt && p.session.hintsUsed === 0) },
      ...
      void client.from("user_achievements").upsert(progress.map((item) => ({ ...item, user_id: userId })));
    }, 700);
    return () => clearTimeout(timeout);
  }, [completed, placed, puzzles, session, xp]);
  ...
  useEffect(() => {
    void authenticateGamePlatform().then((player) => {
      ...
      void reportPlatformAchievement("no_hints", puzzles.some((p) => p.session.completedAt && p.session.hintsUsed === 0) ? 100 : 0);
  ...
  }, [completed, placed, puzzles]);

  const signIn = useCallback(async (provider: "google" | "azure") => {
    setError(null);
    ...
    try {
      ...
      if (result.type !== "success") {
        if (result.type !== "cancel" && result.type !== "dismiss") throw new Error("AUTH_CALLBACK_FAILED");
        return;
      }
      const callback = new URL(result.url);
      const fragment = new URLSearchParams(callback.hash.replace(/^#/, ""));
      const callbackErrorCode = callback.searchParams.get("error_code")
        ?? callback.searchParams.get("error")
        ?? fragment.get("error_code")
        ?? fragment.get("error");
      const callbackError = callback.searchParams.get("error_description") ?? fragment.get("error_description");
      if (callbackErrorCode || callbackError) throw new Error(callbackErrorCode ?? callbackError ?? "AUTH_CALLBACK_FAILED");
      ...
      const accessToken = fragment.get("access_token") ?? callback.searchParams.get("access_token");
      const refreshToken = fragment.get("refresh_token") ?? callback.searchParams.get("refresh_token");
      if (!accessToken || !refreshToken) throw new Error("OAuth did not return a valid app session.");
      ...
      if (message === "AUTH_TIMEOUT") {
        setError(t("O login demorou demais. Verifique a conexão e tente novamente.", "Sign-in took too long. Check your connection and try again."));
      } else if (message === "signup_disabled" || /signup|signups|cadastro/i.test(message)) {
        setError(t("Novos usuários estão desativados no Supabase. Ative ‘Allow new users to sign up’ e tente novamente.", "New users are disabled in Supabase. Enable ‘Allow new users to sign up’ and try again."));
      } else if (message === "AUTH_CALLBACK_FAILED") {
        setError(t("O login não conseguiu retornar ao Pieceful. Verifique as Redirect URLs do Supabase.", "Sign-in could not return to Pieceful. Check the Supabase Redirect URLs."));
      } else {
        setError(message);
      }
    } finally {
  ...
  const updateProfile = useCallback(async (input: Pick<PlayerProfile, "displayName" | "avatarUrl" | "bio">) => {
  ...
  const value = useMemo<SocialState>(() => ({
    ...
    uploadAvatar,
    refreshFriends,
  }), [busy, error, friends, profile, ready, refreshFriends, session, signIn, signOut, updateProfile, uploadAvatar, xp]);
